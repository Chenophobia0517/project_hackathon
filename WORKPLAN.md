...
### TD1. Web Reader 正文抽取的实现方式
建议：**自研轻量抽取**（去 script/style/nav，取文本密度最高的主体块，~百行，零依赖）。备选：打包 Mozilla Readability.js（更稳但 +~100KB 与适配工作）。

### TD2. 来源类型分类的策略
建议：**域名规则优先 + LLM 兜底**（gov.cn/.edu/arxiv/主流媒体表直接命中，未知域批量交 LLM）。备选：全 LLM 判定（简单但费配额）。

### TD3. 来源白名单的配置形态
建议：**V2.0 内置默认四级白名单**（代码内 seed 表），不做设置 UI。备选：options 页可编辑（工作量 +半天）。

### TD4. 自动扫描是否自动溯源
建议：**扫描=发现+分类+定位（不自动搜索验证），溯源由点击触发（单 Claim 完整管线）**——守住成本与"不后台监听"红线；§14 Demo 用"点击一条→完整溯源"呈现。备选：概览页提供「批量溯源前 N 条」按钮（演示冲击力强，配额消耗大，可作为演示彩蛋后加）。

### TD5. 悬浮球尺寸回调
V1.6 你要求放大到 84px；本升级要求"适当缩小、降低侵入"。建议：**64px 折中**（比原始 42px 大、比当前小）+ 拖动/记忆能力弥补存在感。请确认具体值。
